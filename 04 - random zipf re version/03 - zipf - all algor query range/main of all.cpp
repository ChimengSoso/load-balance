...
int zipf(double alpha, int n) {
    ...
    if ((zipf_value >=1) && (zipf_value <= n)) return (int)zipf_value;
    else {
        printf("Error index n = %d, zipf_value = %.2lf\n", n, zipf_value);
    ...
    while (T--) {
        ...
        */
        ...
        for (int i = 0; i < N_thing; ++i) {
            int idx = zipf(alphaZipf, n) - 1; // random [0, n-1] using zipf random
            idx = shuf_tnk[idx];
        ...
        for (int i_range = 0; i_range < size_itv; ++i_range) {
            ...
            for (int t = 1; t <= numQuery; ++t) {
                // random in range [1, N_thing-range+1];
                if (N_thing < range) continue;
